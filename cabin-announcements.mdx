...
For FNX-style packs, select the folder named `Announcements`, not the aircraft package folder above it.
...
| Use Default folder if airline files are missing | Allows FlyHub to fall back to `Default` when the selected airline or operation pack does not contain a matching file. |
...
2. Airline plus operation folder, such as `UAE_Cargo`.
3. Airline folder, such as `UAE`.
4. Operation folder, such as `PAX`, `Cargo`, or `VIP`.
5. `Default`, if default fallback is enabled.

If no matching file is found, FlyHub skips that announcement.

## File Naming Rules

Every file must start with a known announcement name. Optional tags go in square brackets after the name.
...
1. Create a root folder named `Announcements`.
2. Inside it, create a folder for your pack, such as `Default`, `AAL`, `UAE_Cargo`, or `VIP`.
...
7. Select the `Announcements` root folder.
8. Click Rescan.
...
| Generic Default file plays instead of airline file | Add the missing file to the airline pack or disable Default fallback. |
...
- Keep a `Default` folder with generic fallbacks.
